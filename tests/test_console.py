...
from hypermodern_python_tutorial import console
...
def mock_wikipedia_random_page(mocker: MockFixture) -> Mock:
    ...
    return mocker.patch("hypermodern_python_tutorial.wikipedia.random_page")
...
def test_main_uses_en_wikipedia_ord(runner: CliRunner, mock_requests_get: Mock) -> None:
...
@pytest.mark.e2e
def test_main_succeeds_int_production_env(runner: CliRunner) -> None:
    """It exits with status code of zero (end-to-end)."""
    result = runner.invoke(console.main)
    assert result.exit_code == 0
